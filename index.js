...
.then(() => {
  message.channel.awaitMessages(response => response.content.toLowerCase() === 'yes', 'no', {
    max: 1,
    time: 30000,
    errors: ['time'],
  })
  .then((collected.first().content) => {
  ...
  .then((collected.second().content) => {
     // message.channel.send(`You have been demonetised as your response message was: ${collected.second().content}`);
          message.channel.send({embed: {
        color: 15844367,
